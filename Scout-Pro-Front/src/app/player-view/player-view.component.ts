...
export class PlayerViewComponent implements OnInit {
  ...
  fetchPlayerProfile(): void {
    ...
    const apiUrl = this.playerId
      ? `http://localhost:8000/api/player/${this.playerId}`
      : 'http://localhost:8000/api/player/profile';

    this.http.get<{ data: Player; age: number; videos: Video[] }>(apiUrl, { headers }).subscribe({
  next: (response) => {
    this.playerData = {
      ...response.data,
      age: response.age
  ...
  get playerImageUrl(): string {
    return this.playerData?.profile_image
      ? `http://localhost:8000/storage/${this.playerData.profile_image}`
